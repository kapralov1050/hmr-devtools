...
const TOOLS: readonly ManifestTool[] = [
    ...
    {
        name: 'browser_dom',
        description: `Snapshot of DOM — compact (~80 lines, indexed interactive elements with focus/context folding) or raw (recursive tree with attrs/computed styles, viewport filter). Endpoint ${agentDomEndpoint}.`,
        endpoint: `GET ${agentDomEndpoint}`,
        input_schema: {
            type: 'object',
            properties: {
                format: {type: 'string', enum: ['compact', 'raw'], default: 'compact'},
                selector: {type: 'string', description: 'CSS selector; defaults to document.body'},
                focus: {type: 'integer', description: 'Compact only: idx of focused interactive element (adds `→ ` marker)'},
                context: {type: 'integer', description: 'Compact only: lines around focus (default 4)'},
                depth: {type: 'integer', description: 'Raw only: recursion depth (default 5)'},
                ...
                maxSize: {type: 'integer', description: 'Max response size in bytes (default 500000)'},
                instance: {type: 'string', description: 'Required when >1 browser instances'},
            },
        },
    },
    {
